...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function renderAddTurfForm(container, editTurfData = null) {
      ...
      <div class="owner-header">
        <div>
          ...
          <p style="color:var(--text-muted); font-size:0.95rem;">
            List your sports venue on PlaySlot with instant slot booking for thousands of players
          </p>
        </div>
        ...
        <form id="ownerAddTurfForm">
          ...
          <div style="display:grid; grid-template-columns:1fr 1fr 1fr; gap:16px; margin-bottom:18px;">
            <div class="form-group">
              ...
              <select id="addTurfCity" class="form-select" required>
                ...
                <option value="Delhi">Delhi / Gurugram</option>
              </select>
            </div>
            <div class="form-group">
          ...
          <div class="form-group" style="margin-bottom:24px;">
            ...
            <div style="display:grid; grid-template-columns:repeat(3, 1fr); gap:12px;">
              ...
              <label style="display:flex; align-items:center; gap:8px; font-size:0.9rem; cursor:pointer;">
                <input type="checkbox" name="facility" value="Seating" checked> 🪑 Seating
              </label>
              <label style="display:flex; align-items:center; gap:8px; font-size:0.9rem; cursor:pointer;">
                <input type="checkbox" name="facility" value="Equipment Rental" checked> 🏏 Equipment Rental
              </label>
            </div>
          </div>
    ...
    document.getElementById('ownerAddTurfForm').addEventListener('submit', (e) => {
      ...
      PlaySlotApp.showToast(`Turf "${created.name}" created successfully!`, 'success');
      ...
      <div class="table-card">
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:20px; flex-wrap:wrap; gap:12px;">
          <div>
            ...
            <p style="font-size:0.85rem; color:var(--text-muted);">Click on any slot to toggle its availability or modify price</p>
          </div>
        ...
        <div id="ownerSlotGridDisplay" style="display:grid; grid-template-columns:repeat(3, 1fr); gap:16px;">
          ...
          <div style="display:flex; justify-content:space-between; align-items:center; margin-top:4px;">
            ...
            <div style="display:flex; gap:6px;">
              ${slot.status === 'Available' ? `
                <button onclick="window.changeSlotState('${activeTurfId}', '${activeDate}', '${slot.time}', 'Blocked')" class="btn btn-outline-dark btn-sm" title="Block for maintenance">Block</button>
              ` : (slot.status === 'Blocked' ? `
              ...
              ` : '<span style="font-size:0.75rem; color:var(--text-muted);">Customer Booked</span>')}
            </div>
          </div>
        </div>
      `).join('');
  ...
  window.executeAddCustomSlot = (turfId, date) => {
    ...
    window.PlaySlotData.addCustomSlot(turfId, date, time, price);
    PlaySlotApp.closeModal();
    PlaySlotApp.showToast(`Slot "${time}" added!`, 'success');
    renderSlotManagement(document.getElementById('ownerSectionContainer'), turfId);
  };

  // --- 5. Owner Bookings Management ---
  function renderOwnerBookings(container) {
    ...
    container.innerHTML = `
      <div class="owner-header">
        <div>
          <h1 class="owner-title" style="font-size:1.8rem; font-weight:900;">🎟️ Turf Reservations & Booking Log</h1>
              ...
              ${bookings.map(b => `
                <tr>
                  ...
                  <td>
                    ...
                    ` : '<span style="color:var(--text-muted); font-size:0.82rem;">Archived</span>'}
                  </td>
                </tr>
              `).join('')}
            </tbody>
