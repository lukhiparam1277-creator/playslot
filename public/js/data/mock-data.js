...
const PlaySlotData = (() => {
  ...
  return {
    ...
    updateSport: (id, sportData) => {
      ...
      if (idx !== -1) {
        sports[idx] = {
          ...sports[idx],
          ...sportData,
          startingPrice: parseInt(sportData.startingPrice) || sports[idx].startingPrice,
          id: sports[idx].id
        };
        setStorage(STORAGE_KEYS.SPORTS, sports);
        return sports[idx];
    ...
    saveTurf: (turfData) => {
      const turfs = getStorage(STORAGE_KEYS.TURFS, defaultTurfs);
      const existingIdx = turfData.id ? turfs.findIndex(t => t.id === turfData.id) : -1;
      ...
      if (existingIdx !== -1) {
        ...
        turfs[existingIdx] = {
          ...turfs[existingIdx],
          ...turfData,
          pricePerHour: parseInt(turfData.pricePerHour) || turfs[existingIdx].pricePerHour
        };
        setStorage(STORAGE_KEYS.TURFS, turfs);
        return turfs[existingIdx];
      }
      ...
      const newTurf = {
        ...
        rules: turfData.rules || ['Rubber studs or non-marking sports shoes only.', 'Report 10 mins before slot.'],
    ...
    updateTurf: (id, turfData) => {
      const turfs = getStorage(STORAGE_KEYS.TURFS, defaultTurfs);
      const idx = turfs.findIndex(t => t.id === id);
      if (idx !== -1) {
        turfs[idx] = {
          ...turfs[idx],
          ...turfData,
          pricePerHour: turfData.pricePerHour !== undefined ? parseInt(turfData.pricePerHour) : turfs[idx].pricePerHour,
          id: turfs[idx].id
        };
        setStorage(STORAGE_KEYS.TURFS, turfs);
        return turfs[idx];
    ...
    addCustomSlot: (turfId, dateStr, slotData) => {
      const key = `${turfId}_${dateStr}`;
      const allSlotsMap = getStorage(STORAGE_KEYS.SLOTS, {});
      const slots = allSlotsMap[key] || PlaySlotData.getSlotsForTurfAndDate(turfId, dateStr);
      const newSlot = {
        id: 'slot-' + Date.now(),
        time: slotData.time,
    ...
    updateSlot: (turfId, dateStr, slotTime, slotData) => {
      const key = `${turfId}_${dateStr}`;
      const allSlotsMap = getStorage(STORAGE_KEYS.SLOTS, {});
      const slots = allSlotsMap[key] || PlaySlotData.getSlotsForTurfAndDate(turfId, dateStr);
      const slot = slots.find(s => s.time === slotTime || s.id === slotTime);
      if (slot) {
        if (slotData.time) slot.time = slotData.time;
        if (slotData.price !== undefined) slot.price = parseInt(slotData.price);
        if (slotData.status) slot.status = slotData.status;
        allSlotsMap[key] = slots;
        setStorage(STORAGE_KEYS.SLOTS, allSlotsMap);
        return slot;
      }
    ...
    deleteSlot: (turfId, dateStr, slotIdentifier) => {
      const key = `${turfId}_${dateStr}`;
      const allSlotsMap = getStorage(STORAGE_KEYS.SLOTS, {});
      let slots = allSlotsMap[key] || PlaySlotData.getSlotsForTurfAndDate(turfId, dateStr);
      slots = slots.filter(s => s.time !== slotIdentifier && s.id !== slotIdentifier);
      allSlotsMap[key] = slots;
      setStorage(STORAGE_KEYS.SLOTS, allSlotsMap);
      return true;
    },

    // --- Bookings Methods (CRUD) ---
    getBookings: (filters = {}) => {
    ...
    updateBooking: (id, bookingData) => {
      const bookings = getStorage(STORAGE_KEYS.BOOKINGS, defaultBookings);
      const idx = bookings.findIndex(b => b.id === id || b.bookingId === id);
      if (idx !== -1) {
        bookings[idx] = {
          ...bookings[idx],
          ...bookingData,
    ...
    deleteBooking: (id) => {
      const bookings = getStorage(STORAGE_KEYS.BOOKINGS, defaultBookings).filter(b => b.id !== id && b.bookingId !== id);
      setStorage(STORAGE_KEYS.BOOKINGS, bookings);
      return true;
    },
    ...
    deleteUser: (id) => {
      const users = getStorage(STORAGE_KEYS.USERS, defaultUsers).filter(u => u.id !== id);
      setStorage(STORAGE_KEYS.USERS, users);
      return true;
    },
    ...
    getOwnerStats: (ownerId) => {
      const myTurfs = PlaySlotData.getTurfs({ ownerId: ownerId || 'owner-1' });
      return {
